...
public class UploadParkLogic {
	...
	public static ReturnResult savainfoUserLoginLog(InfoUserLoginLogRepository infoUserLoginLogpRepository,InfoLogUploadRepository infoLogUploadRepository,String json) throws JsonParseException, JsonMappingException, IOException, ParseException{
		ReturnHead rhead=new ReturnHead();
		ReturnResult result= new ReturnResult();
		result.setHead(rhead);
		ObjectMapper mapper=new ObjectMapper();
		UploadUserLoginLog infoEmp=mapper.readValue(json, UploadUserLoginLog.class);
		UploadHead head=infoEmp.getHead();
		UploadUserLoginLogParameter parameter=infoEmp.getParameter();
		String userCode=parameter.getUserCode();
		
		InfoUserLoginLog infoUserLoginLog=new InfoUserLoginLog();
		infoUserLoginLog.setEmpName(parameter.getEmpName());
		infoUserLoginLog.setUserEmpNo(parameter.getEmpNo());
		infoUserLoginLog.setId(null);
		infoUserLoginLog.setOpType(parameter.getOpType());
		infoUserLoginLog.setOpTime(CustomTime.parseTime(parameter.getOpTime()));
		infoUserLoginLog.setParkId(head.getParkId());
		infoUserLoginLog.setUpdateTime(new Date());
		infoUserLoginLog.setUserCode(parameter.getUserCode());
		infoUserLoginLog.setUserType(parameter.getUserType());
		infoUserLoginLog.setUserTypeName(parameter.getUserTypeName());
		infoUserLoginLog.setParkId(head.getParkId());
		InfoUserLoginLog m = infoUserLoginLogpRepository.save(infoUserLoginLog);
		if (Objects.isNull(m)) {
			rhead.setCode("101");
