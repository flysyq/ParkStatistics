...
public class InfoMailLog {
	...
	@RequestMapping(value="infomaillog/list",method=RequestMethod.GET)
	public String list(TimeCommonForm form,Model model,HttpServletRequest request){
		String cousql="select count(*) cou from info_mail_log where start_time>'"+form.getStart_date()+"' and start_time<'"+form.getEnd_date()+"'";
		...
		String sql="select * from info_mail_log where start_time>'"+form.getStart_date()+"' and start_time<'"+form.getEnd_date()+"' limit " + ((page.getPage() - 1) * page.getPage_size()) + "," + page.getPage_size();
		...
		session.setAttribute("childrentitle", "邮件发送日志");
		String forward="infomaillog/list";
		return forward;
	}
}
