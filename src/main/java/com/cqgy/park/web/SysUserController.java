...
public class SysUserController {
	...
	public String list(UserListForm form,Long del_id,HttpServletRequest request,Model model){
		...
		String select = "select * from sys_user limit "+((page.getPage() - 1) * page.getPage_size()) + "," + page.getPage_size();
		String where = "";
	...
	public String edit(Long id,Model model){
		SysUser sysUser=new SysUser(new Integer(0).longValue(), "", "", "", 0,"",0);
	...
	public String sava(Long id,String logincode,String loginpassword,String name,Integer mailflag,String email,Model model,HttpServletRequest request){
		...
		if (!Strings.isNullOrEmpty(loginpassword)) {
			...
			if (!Strings.isNullOrEmpty(logincode)) {
				String sql="select * from sys_user where login_code='"+logincode+"'";
				List<Map<String,Object>> list = jdbcTemplate.queryForList(sql);
				if (!list.isEmpty()) {
					sysUser.setLoginPassword((String) list.get(0).get("login_password"));
				}
			}
		}
