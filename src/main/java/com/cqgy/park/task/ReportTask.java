...
public class ReportTask {
	...
	public void genReport(){
		...
		String searchStartTime = ReportTool.getSearchTime(jdbcTemplate);
		try {
			InfoDutyShriftReportLog log =new InfoDutyShriftReportLog(null,new Date(),CustomTime.parseTime(searchStartTime),1);
			
			String sql = "select park_id,user_emp_no,date_format(op_time,'%Y-%m-%d %T') op_time from info_user_login_log a"
					+ " where user_type=1 and a.op_time>='"+searchStartTime+"' order by park_id,op_time asc";
			...
			for(int i=1;i<list.size();i++){
				String tempEmpNo = (String)list.get(0).get("user_emp_no");
				String tempParkId = (String)list.get(i).get("park_id");
				if(!(report.getEmpNo().equals(tempEmpNo) && report.getParkId().equals(tempParkId))){
					report.setDownTime(CustomTime.parseTime((String)list.get(i).get("on_time")));
					//获取进入车库的车辆数量。
